...
SELECT VALIDACION_CUENTA('usuario1@example.com','password1');
/*
funciones
...
CREATE DEFINER=`root`@`localhost` FUNCTION `sum_compra_producto`(codigo int) RETURNS int(11)
BEGIN
	DECLARE SUMA INTEGER;
    SET SUMA = (SELECT SUM(CANTPRODUCTO) FROM COMPRAS WHERE COD_PRODUCTO = CODIGO);
RETURN SUMA;
END

CREATE DEFINER=`root`@`localhost` FUNCTION `VALIDACION_CUENTA`(CORREO1 VARCHAR(250), PASS1 VARCHAR(250)) RETURNS tinyint(1)
BEGIN
	DECLARE CORREO2 VARCHAR(250);
	DECLARE PASS2 VARCHAR(250);
    ...
    WHILE CONTADOR <= REGISTROS AND VALIDACION = FALSE DO
		SET CORREO2 = (SELECT CORREO FROM USUARIOS WHERE COD_USUARIO = CONTADOR);
        SET PASS2 = (SELECT PASS FROM USUARIOS WHERE COD_USUARIO = CONTADOR);
        
        IF CORREO1 = CORREO2 AND PASS1 = PASS2 THEN
			SET VALIDACION = TRUE;
		ELSE
			SET VALIDACION = FALSE;
        END IF;
